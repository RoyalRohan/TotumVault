    ...
    #[cfg(target_os = "windows")]
    {
        if let Some(window) = _app.get_webview_window("main") {
            if let Ok(hwnd) = window.hwnd() {
                ...
                unsafe {
                    type SetWindowDisplayAffinityFn = extern "system" fn(*mut std::ffi::c_void, u32) -> i32;
                    let user32_name: Vec<u16> = "user32.dll\0".encode_utf16().collect();
                    let h_module = windows_sys::Win32::System::LibraryLoader::LoadLibraryW(user32_name.as_ptr());
                    if !h_module.is_null() {
                        let proc_name = std::ffi::CString::new("SetWindowDisplayAffinity").unwrap();
                        let proc = windows_sys::Win32::System::LibraryLoader::GetProcAddress(h_module, proc_name.as_ptr() as _);
                        if let Some(func) = proc {
                            let set_display_affinity: SetWindowDisplayAffinityFn = std::mem::transmute(func);
                            let _ = set_display_affinity(hwnd.0 as _, affinity);
                        }
                    }
                }
            }
